...
def unmarshal_message(message: bytes) -> list:
    """
    Steps:
    Check len and divide on 32 to get number of partitions
    every 32 bytes unmarshal them and append to big list
    ...
    Bytes[14:22] The exchange rate is 64-bit floating point number represented in IEEE 754 binary64 
    little-endian format. The rate is number of currency2 units to be exchanged per one unit of currency1. So, for example, if currency1 is USD and currency2 is JPY, we'd expect the exchange rate to be around 100.
